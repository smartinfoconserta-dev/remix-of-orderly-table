...
-- ========================================
-- Secure RLS for operational tables
-- Remove public write access, keep anon SELECT (for Realtime)
-- Add authenticated policies (master + store_members)
-- Create SECURITY DEFINER RPCs for device writes
-- ========================================

-- 1. PEDIDOS
DROP POLICY IF EXISTS "pedidos_public_all" ON public.pedidos;

CREATE POLICY "pedidos_auth_master" ON public.pedidos FOR ALL TO authenticated
  USING (is_master(auth.uid())) WITH CHECK (is_master(auth.uid()));

CREATE POLICY "pedidos_auth_members" ON public.pedidos FOR ALL TO authenticated
  USING (store_id IN (SELECT get_user_store_ids(auth.uid())))
  WITH CHECK (store_id IN (SELECT get_user_store_ids(auth.uid())));

CREATE POLICY "pedidos_anon_select" ON public.pedidos FOR SELECT TO anon
  USING (true);

-- 2. FECHAMENTOS
DROP POLICY IF EXISTS "fechamentos_public_insert" ON public.fechamentos;
DROP POLICY IF EXISTS "fechamentos_public_read" ON public.fechamentos;
DROP POLICY IF EXISTS "fechamentos_public_update" ON public.fechamentos;

CREATE POLICY "fechamentos_auth_master" ON public.fechamentos FOR ALL TO authenticated
  USING (is_master(auth.uid())) WITH CHECK (is_master(auth.uid()));

CREATE POLICY "fechamentos_auth_members" ON public.fechamentos FOR ALL TO authenticated
  USING (store_id IN (SELECT get_user_store_ids(auth.uid())))
  WITH CHECK (store_id IN (SELECT get_user_store_ids(auth.uid())));

CREATE POLICY "fechamentos_anon_select" ON public.fechamentos FOR SELECT TO anon
  USING (true);

-- 3. MOVIMENTACOES_CAIXA
DROP POLICY IF EXISTS "movimentacoes_public_insert" ON public.movimentacoes_caixa;
DROP POLICY IF EXISTS "movimentacoes_public_read" ON public.movimentacoes_caixa;

CREATE POLICY "movimentacoes_auth_master" ON public.movimentacoes_caixa FOR ALL TO authenticated
  USING (is_master(auth.uid())) WITH CHECK (is_master(auth.uid()));

CREATE POLICY "movimentacoes_auth_members" ON public.movimentacoes_caixa FOR ALL TO authenticated
  USING (store_id IN (SELECT get_user_store_ids(auth.uid())))
  WITH CHECK (store_id IN (SELECT get_user_store_ids(auth.uid())));

CREATE POLICY "movimentacoes_anon_select" ON public.movimentacoes_caixa FOR SELECT TO anon
  USING (true);

-- 4. EVENTOS_OPERACIONAIS
DROP POLICY IF EXISTS "eventos_public_insert" ON public.eventos_operacionais;
DROP POLICY IF EXISTS "eventos_public_read" ON public.eventos_operacionais;

CREATE POLICY "eventos_auth_master" ON public.eventos_operacionais FOR ALL TO authenticated
  USING (is_master(auth.uid())) WITH CHECK (is_master(auth.uid()));

CREATE POLICY "eventos_auth_members" ON public.eventos_operacionais FOR ALL TO authenticated
  USING (store_id IN (SELECT get_user_store_ids(auth.uid())))
  WITH CHECK (store_id IN (SELECT get_user_store_ids(auth.uid())));

CREATE POLICY "eventos_anon_select" ON public.eventos_operacionais FOR SELECT TO anon
  USING (true);

-- 5. ESTADO_CAIXA
DROP POLICY IF EXISTS "estado_caixa_public_insert" ON public.estado_caixa;
DROP POLICY IF EXISTS "estado_caixa_public_read" ON public.estado_caixa;
DROP POLICY IF EXISTS "estado_caixa_public_update" ON public.estado_caixa;

CREATE POLICY "estado_caixa_auth_master" ON public.estado_caixa FOR ALL TO authenticated
  USING (is_master(auth.uid())) WITH CHECK (is_master(auth.uid()));

CREATE POLICY "estado_caixa_auth_members" ON public.estado_caixa FOR ALL TO authenticated
  USING (store_id IN (SELECT get_user_store_ids(auth.uid())))
  WITH CHECK (store_id IN (SELECT get_user_store_ids(auth.uid())));

CREATE POLICY "estado_caixa_anon_select" ON public.estado_caixa FOR SELECT TO anon
  USING (true);

-- ========================================
-- SECURITY DEFINER RPCs for device writes
-- ========================================
...
-- RPC: Update pedido
CREATE OR REPLACE FUNCTION public.rpc_update_pedido(_id text, _store_id uuid, _updates jsonb)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  UPDATE public.pedidos SET
    pronto = CASE WHEN _updates ? 'pronto' THEN (_updates->>'pronto')::bool ELSE pronto END,
    status_balcao = CASE WHEN _updates ? 'status_balcao' THEN _updates->>'status_balcao' ELSE status_balcao END,
    cancelado = CASE WHEN _updates ? 'cancelado' THEN (_updates->>'cancelado')::bool ELSE cancelado END,
    cancelado_em = CASE WHEN _updates ? 'cancelado_em' THEN _updates->>'cancelado_em' ELSE cancelado_em END,
    cancelado_motivo = CASE WHEN _updates ? 'cancelado_motivo' THEN _updates->>'cancelado_motivo' ELSE cancelado_motivo END,
    cancelado_por = CASE WHEN _updates ? 'cancelado_por' THEN _updates->>'cancelado_por' ELSE cancelado_por END,
    motoboy_nome = CASE WHEN _updates ? 'motoboy_nome' THEN _updates->>'motoboy_nome' ELSE motoboy_nome END,
    forma_pagamento_delivery = CASE WHEN _updates ? 'forma_pagamento_delivery' THEN _updates->>'forma_pagamento_delivery' ELSE forma_pagamento_delivery END,
    troco_para_quanto = CASE WHEN _updates ? 'troco_para_quanto' THEN (_updates->>'troco_para_quanto')::numeric ELSE troco_para_quanto END,
    total = CASE WHEN _updates ? 'total' THEN (_updates->>'total')::numeric ELSE total END,
    itens = CASE WHEN _updates ? 'itens' THEN _updates->'itens' ELSE itens END,
    updated_at = now()
  WHERE id = _id AND store_id = _store_id;
END;
$$;

-- RPC: Insert fechamento
...
-- RPC: Update fechamento
CREATE OR REPLACE FUNCTION public.rpc_update_fechamento(_id text, _store_id uuid, _updates jsonb)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
